...
/**
 * Late chunking pipeline with 1024 token maximum
 *
 * Strategy:
 * - Embeds the FULL DEBATE first to capture global context
 * - Then chunks the debate into smaller pieces
 ...
 */
export class LateChunkingPipeline1024 extends ChunkingPipeline {
  readonly strategyName: ChunkingStrategy = 'late_chunking_1024';
  ...
  /**
   * Process a debate using late chunking strategy
  ...
  async process(debate: Debate): Promise<Chunk[]> {
    ...
    console.log(`[Late Chunking 1024] Embedding full debate: ${debate.title}`);
    ...
    console.log(`[Late Chunking 1024] Debate embedding complete (${debateEmbedding.length} dimensions)`);
    ...
    console.log(`[Late Chunking 1024] Created ${chunks.length} chunks with contextual blending`);
